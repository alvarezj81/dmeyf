...
hs <- makeParamSet( 
         makeNumericParam("learning_rate",    lower=    0.025 , upper=    0.038), 
         makeNumericParam("feature_fraction", lower=    0.55  , upper=    0.88),
         makeIntegerParam("min_data_in_leaf", lower=  1900L   , upper= 3300L), 
         makeIntegerParam("num_leaves",       lower=    50L   , upper= 850L)
...
# hs <- makeParamSet( 
#   makeNumericParam("learning_rate",    lower=    0.01 , upper=    0.09), #cambio lower de 0.02 a 0.01 y upper de 0.1  0.09
#   makeNumericParam("feature_fraction", lower=    0.1  , upper=    1.0),
#   makeIntegerParam("min_data_in_leaf", lower=  1000L   , upper= 6500L), #cambio lo de 100L a 1000L y up de 8000 a 6500
#   makeIntegerParam("num_leaves",       lower=    20L   , upper= 1024L)
...
#   makeIntegerParam("min_data_in_leaf", lower=  1000L   , upper= 6500L), 
...
#   
#)
# para la tercera deje hasta num leaves

#AGREGUÉ TODAS LAS QUE TENIA ANTES. AsÍ TENGO MÄS MODELOS DISTINTOS
campos_malos  <- c("ccajas_transacciones", "Master_mpagominimo", "internet","mcaja_ahorro_dolares", "mpayroll", "mpagomiscuentas", "ctransferencias_recibidas", "mtransferencias_recibidas", "tmobile_app", "cmobile_app_trx", "Master_mfinanciacion_limite", "Master_Fvencimiento", "Master_Finiciomora", "Master_mconsumosdolares", "Master_madelantodolares", "Master_fultimo_cierre", "Master_mpagado", "Master_mpagosdolares", "Master_mpagominimo", "Visa_mfinanciacion_limite", "Visa_Finiciomora", "Visa_msaldodolares", "Visa_mconsumosdolares", "Visa_madelantodolares", "Visa_fultimo_cierre", "Visa_mpagado", "Visa_mpagosdolares")   #aqui se deben cargar todos los campos culpables del Data Drifting
#PONGO MI SEGUNDA SEMILLA AHORA. CORRIDA 4ta
ksemilla_azar  <- 293  #Aqui poner la propia semilla
#------------------------------------------------------------------------------
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  param_basicos  <- list( objective= "binary",
                          ...
                          #bagging_fraction = 0.632, #AGREGO ESTO PARA QUE HAGA RF ENTRE 0 y 0.99
                          #boosting="rf",
                          #bagging_frecuency = 2
                          extra_trees = TRUE,        #AGREGO ESTO PARA EXTRA TREES
                          num_iterations= 9999,   #un numero muy grande, lo limita early_stopping_rounds
                          force_row_wise= TRUE    #para que los alumnos no se atemoricen con tantos warning
                        )
###AGREGO ESTO PARA QUE FUNCIONE EL MAX_BIN COMO HIPERPARAMETRO
